...
//            add_filter('wp_mail_content_type', create_function('', 'return "text/html"; '));
            add_filter ( 'wp_mail_content_type', 'Apsl_content_type' );
            function Apsl_content_type() {
                return "text/html";
            }
...
if (!class_exists('APSL_Class')) {
    ...
    class APSL_Class {
        ...
        function __construct() {
            ...
            add_action('template_redirect', array($this, 'get_insta_code'));
            add_action('template_redirect', array($this, 'get_yahoo_code'));
            ...
            add_action('admin_post_apsl_add_new_network_array', array($this, 'apsl_add_new_network_array'));
        ...
        /*
         * Function to add new social links. Since Version >= 2.0.0
         */

        function apsl_add_new_network_array() {
            ...
            if (!empty($_GET) && wp_verify_nonce($nonce, 'apsl-restore-default-settings-nonce')) {
                ...
                $options['network_ordering'] = array_merge($options['network_ordering'], $social_networks_new_array);
                foreach ($options as $key => $val) {
                    if ($key == 'network_ordering') {
                        $options[$key] = $options['network_ordering'];
                    }
                }
                update_option(APSL_SETTINGS, $options);
                $_SESSION['apsl_message'] = __('New Social Network Fields Added Successfully.', APSL_TEXT_DOMAIN);
        ...
        //get code value for instagram
        function get_insta_code(){
            if(strpos( $_SERVER['REQUEST_URI'], "apsl_instagram_check?code") !== false){
            $classname='APSL_Functions';
            $socialmedia = new $classname();
            $userdetail = $socialmedia->instagramLogin();
            $result=$userdetail;
            include(APSL_PLUGIN_DIR . 'inc/frontend/login-check/instagram-login-check-response.php');
            }
        }
        //get code value for yahoo
        function get_yahoo_code(){
            if(strpos( $_SERVER['REQUEST_URI'], "apsl_yahoo_check?code") !== false){
            $classname='APSL_Functions';
            $socialmedia = new $classname();
            $userdetail = $socialmedia->YahooLogin();
            $result=$userdetail;
            include(APSL_PLUGIN_DIR . 'inc/frontend/login-check/yahoo-login-check-response.php');
            }
        }
        ...
        function add_apsl_menu() {
            add_menu_page('AccessPress Social Login', __('AccessPress Social Login', APSL_TEXT_DOMAIN), 'manage_options', APSL_TEXT_DOMAIN, array($this, 'main_page'), APSL_IMAGE_DIR . '/icon.png');
        }
        ...
        function register_admin_assets() {
            wp_enqueue_style('fontawsome-css', '//maxcdn.bootstrapcdn.com/font-awesome/4.7.0/css/font-awesome.min.css', '', APSL_VERSION);
        ...
        function register_frontend_assets() {
            ...
            wp_localize_script('apsl-frontend-js', 'apsl_ajax_object', array('ajax_url' => admin_url('admin-ajax.php')));
        ...
        public function social_buttons_in_checkout($template_name) {
            if ($template_name == 'checkout/form-login.php') {
                ...
                if (!is_user_logged_in()) {
                    ?>
                    <p class="woocommerce-info"><?php echo $options['apsl_title_text_field']; ?> <a href="#" class="show-apsl-container"><?php _e('Click here to login', APSL_TEXT_DOMAIN); ?></a> </p>
                    <form class="login apsl-container" style="display: none;">
                        <?php echo do_shortcode("[apsl-login login_text='{$login_text}']"); ?>
                    </form>
                    <?php
                }
            }
        ...
        //function to add login form shortcode
