...
if (is_user_logged_in()) {
    ...
    ?><div class="<?php if (is_rtl()) { ?> apsl-rtl-wrap <?php } ?> user-login"><?php _e('Welcome', APSL_TEXT_DOMAIN); ?> <b><?php echo $user_info; ?></b>&nbsp;|&nbsp;<a href="<?php echo $user_logout_url; ?>" title="<?php _e('Logout', APSL_TEXT_DOMAIN); ?>"><?php _e('Logout', APSL_TEXT_DOMAIN); ?></a></div>
    <?php
} else {
    ?>
    <?php
    ...
    <?php $template = isset($attr['template']) ? $attr['template'] : '1'; ?>

    <div class='<?php if (is_rtl()) { ?> apsl-rtl-wrap <?php } ?> apsl-login-with-login-form-networks template-<?php echo $template; ?> clearfix'>
        ...
        <div class='apsl-seperator'><hr><span><?php echo _e('OR', APSL_TEXT_DOMAIN); ?></span></div>
                ...
                ?>
                <?php 
                //$theme = isset($attr['theme']) ? $attr['theme'] : $options['apsl_icon_theme']; // Disabled for the demo used below instead
                 $theme = isset($attr['theme']) ? $attr['theme'] : esc_attr($_POST['login_form_up_theme']);
                 ?>
                <?php $login_text = isset($attr['login_text']) ? $attr['login_text'] : ''; ?>

        <?php echo do_shortcode("[apsl-login theme='$theme' login_text='$login_text' login_redirect_url='$login_redirect_url']"); ?>
    </div>
<?php }